...
// Interface for a Branch, code is optional
interface Branch {
  id: number;
  name: string;
  code?: string | null;
...
// Interface for the form's state, including new file fields
interface FormData {
  name: string;
  phone: string;
  branch_id: string;
...
const PublicStudentRegistration: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.profileImage) {
        ...
        const response = await api.uploadPublicImage(imageFormData);
        profileImageUrl = response.imageUrl;
      }
      if (formData.aadhaarFront) {
        const frontFormData = new FormData();
        frontFormData.append('image', formData.aadhaarFront);
        const response = await api.uploadPublicImage(frontFormData);
        aadhaarFrontUrl = response.imageUrl;
      }
      if (formData.aadhaarBack) {
        const backFormData = new FormData();
        backFormData.append('image', formData.aadhaarBack);
        const response = await api.uploadPublicImage(backFormData);
        aadhaarBackUrl = response.imageUrl;
      }
      ...
      setFormData({
        name: '', phone: '', branch_id: branches.length > 0 ? branches[0].id.toString() : '',
        email: '', address: '', father_name: '', aadhar_number: '', registration_number: '',
        profileImage: null, aadhaarFront: null, aadhaarBack: null
      });
  ...
  const fileInputStyles = "mt-1 block w-full text-sm text-gray-700 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-purple-100 file:text-purple-700 hover:file:bg-purple-200";
      ...
      <div className="max-w-2xl w-full mx-auto bg-white p-8 md:p-10 rounded-xl shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <select name="branch_id" value={formData.branch_id} onChange={handleChange} className={inputStyles} required disabled={branches.length === 0}>
              {branches.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input type="text" name="father_name" value={formData.father_name} onChange={handleChange} className={inputStyles} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input type="text" name="aadhar_number" value={formData.aadhar_number} onChange={handleChange} className={inputStyles} />
            </div>
            <div>
              <label className={labelStyles}>Registration Number</label>
              <input type="text" name="registration_number" value={formData.registration_number} onChange={handleChange} className={inputStyles} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                <label className={labelStyles}>Profile Image (Max Size 200KB)</label>
                <input type="file" name="profileImage" accept="image/*" onChange={handleFileChange} className={fileInputStyles} />
            </div>
            <div>
                <label className={labelStyles}>Aadhaar Front (Max Size 200KB)</label>
                <input type="file" name="aadhaarFront" accept="image/*" onChange={handleFileChange} className={fileInputStyles} />
            </div>
            <div>
                <label className={labelStyles}>Aadhaar Back (Max Size 200KB)</label>
                <input type="file" name="aadhaarBack" accept="image/*" onChange={handleFileChange} className={fileInputStyles} />
            </div>
          </div>
