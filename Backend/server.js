...
app.use(cors({
  origin: function (origin, callback) {
    const allowedOrigins = [
      'http://localhost:8080',
      'https://priyatamlibrary.onrender.com',
      'file://'
    ];
...
// NEW: PUBLIC upload endpoint for the registration page
app.post('/api/public/upload-image', upload.single('image'), async (req, res) => {
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        { resource_type: 'image', folder: 'public_uploads' }, // Optionally use a different folder
        (error, uploadResult) => {
          if (error) return reject(error);
          resolve(uploadResult);
        }
    ...
    if (!result || !result.secure_url) {
      logger.error('Public Cloudinary upload failed, no secure_url returned:', result);
      return res.status(500).json({ message: 'Image upload failed with Cloudinary' });
    }

    logger.info('Public image uploaded successfully to Cloudinary', { imageUrl: result.secure_url });
    res.json({ imageUrl: result.secure_url });

  } catch (error) {
    logger.error('Error uploading public image to Cloudinary:', {
      message: error.message,
      stack: error.stack
    });
    res.status(500).json({ message: 'Server error during image upload', error: error.message });
